...
describe("Album", function 
    ...
    it("creates from data", function () {
        expect(this.album.get('tracks').length).toBe(2);
    });
    ...
    describe("last track", function() {
      it("identifies last track correctly", function() {
...
describe("Player", function() {
  ...
  describe("with no items", function() {
    ...
    it("starts witih album 0", function() {
      expect(this.player.get('currentAlbumIndex')).toEqual(0);
    });

    it("starts witih track 0", function() {
  ...
  describe("with added album", function() {
    ...
    describe("while playing", function() {
      ...
      xit("highlights the first track", function() {
        window.library.reset([{ 'title':  'Album A', 'artist': 'Artist A', 'tracks': [ { 'title': 'Track A', 'url': '/music/Album A Track A.mp3' }, { 'title': 'Track B', 'url': '/music/Album A Track B.mp3' }] }, { 'title': 'Album B', 'artist': 'Artist B', 'tracks': [ { 'title': 'Track A', 'url': '/music/Album B Track A.mp3' }, { 'title': 'Track B', 'url': '/music/Album B Track B.mp3' }]}]);
        console.log(window.library);
        expect(this.el.find('li').first().hasClass('current')).toBeTruthy();
      });
